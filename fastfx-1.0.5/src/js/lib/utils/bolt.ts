...
export const evalTS = <Key extends string & keyof Scripts, Func extends Function & Scripts[Key]>(
	functionName: Key,
	...args: ArgTypes<Func>
...
export const getAppBackgroundColor = () => {
	const { green, blue, red } = JSON.parse(window.__adobe_cep__.getHostEnvironment() as string).appSkinInfo
...
export const subscribeBackgroundColor = (callback: (color: string) => void) => {
	...
	callback(getColor());
	// listen for changes
	csi.addEventListener('com.adobe.csxs.events.ThemeColorChanged', () => callback(getColor()), {});
};
...
export const selectFolder = (dir: string, msg: string, callback: (res: string) => void) => {
...
export const selectFile = (dir: string, msg: string, callback: (res: string) => void) => {
	const result = (window.cep.fs.showOpenDialogEx || window.cep.fs.showOpenDialog)(
		false,
		false,
		msg,
